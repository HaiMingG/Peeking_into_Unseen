...
from DataLoader import KINS_Dataset
...
def make_demo(data_loader, rank, img_index, obj_index, exp_set_up, out_dir=''):
    input_gt_label, bmask_thrd = exp_set_up
    ...
    pred_seg = (pred_segmentation >= bmask_thrd).astype(float)
...
def eval_performance(data_loader, rpn_results, demo=True, category='car', eval_modes=('inmodal', 'amodal'), input_bbox_type='inmodal', input_gt_label=True, search_for_file=True):
    ...
    output = {'average iou': np.mean(iou_perf_cls), 'precision': np.mean((iou_perf_cls >= 0.5).astype(float)), 'num_objects': iou_perf.shape[0]}
    print_line += '|     mIoU: {:6.4f}     '.format(output['average iou'])
    ...
    if demo:
        ...
        for ri in rank_ind:
            if i % 50 == 0 or i < 75:
                make_demo(data_loader, rank=i, img_index=m_dict['img_index'][ri], obj_index=m_dict['obj_index'][ri], exp_set_up=[input_bbox_type, input_gt_label, mask_dict['bmask_thrd']], out_dir=out_dir)
...
if __name__ == '__main__':
    ...
    if MODEL_TYPE == 'E2E':
        pretrain_file = '/home/BLC/Models/E2E/kinsv_final.pth'
    ...
    for input_bbox_type in ['inmodal', 'amodal']:
        ...
        for occ_bound in occ_bounds:
            ...
            with open('/home/../mrcnn_inmodal_kins_RPN.pickle', 'rb') as fh:
