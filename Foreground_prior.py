...
def learn_mix_model_vMF(num_layers=2, num_clusters_per_layer=2, frac_data=1.0, iteration=0, CONTEXT_THRD=0.5):

    # Spectral clustering based on the similarity matrix

    sim_fname = meta_dir + 'init_vgg/' + 'similarity_vgg_pool4_{}/'.format(dataset_train) + 'simmat_mthrh045_K512.pickle'
    ...
    #Dataloader
    data_set = KINS_Compnet_Train_Dataset(height_thrd=75, pad=48)
    ...
    for ii, data in enumerate(data_loader):
        ...
        vc_activation, fg_mask = net.get_vc_activation_with_binary_fg_mask(input.cuda(device_ids[0]), gt_category=categories['train'], use_context_center=(iteration == 0), use_mixture_model=(iteration > 0), context_thrd=CONTEXT_THRD, mmodel_thrd=0, bmask_post_process=True, cntxt_pad=true_pad // feat_stride, low_res=LOW_RES)
    ...
    for kk in range(K):
        ...
        graph_prior(prob_map_fg, prob_map_bg, prior_dir + '{}'.format(kk))

    savename = os.path.join(mixdir, 'mmodel_K{}_FEATDIM{}_{}_specific_view_{}.pickle'.format(K, vc_num, layer, context_cluster))
...
if __name__ == '__main__':
    ...
    try:
        with open(init_dir + 'context_thrds_pig/context_thrd_5.pickle', 'rb') as fh:
    ...
    for it in range(ITERATION):
        ...
        mixdir = init_dir + 'mix_model_vmf_pig_EM_all_context_{}_it{}/'.format(model_tag, it)
        ...
        for num_layers in [3]:
            learn_mix_model_vMF(num_layers=num_layers, num_clusters_per_layer=2, iteration=it, CONTEXT_THRD=context_thrds)
