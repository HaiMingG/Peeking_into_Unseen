...
def eval(model, test_loader, header=''):
    ...
    for index, data in enumerate(test_loader):
        ...
        with torch.no_grad():
            pred_scores, pred_confidence, pred_amodal_bboxes, pred_segmentations = model(org_x=input, bboxes=input_bbox, bbox_type=input_bbox_type, input_label=None, mask_label_training=gt_labels[0].numpy())
        ...
        if index % 10 == 0:
            ...
            visualize_multi(demo_img[0].numpy(), [(pred_segmentations[0]['amodal'] >= 0.01).astype(float), gt_amodal_segmentation[0]],
                            '/home/../temp/{}_{}'.format(header, index), cbar=True)
...
def train(model, train_loader, test_loader, epochs, batch_size, learning_rate, savedir, alpha=3, beta=3, gamma=1, vc_flag=False, mix_flag=False, prior_flag=False):
    ...
    for epoch in range(epochs):
        ...
        for index, data in enumerate(train_loader):
            ...
            pred_likelihood, pred_scores, pred_amodal_bboxes, pred_segmentations = net(org_x=input, bboxes=input_bbox, bbox_type=input_bbox_type, mask_label_training=gt_labels[0].numpy())
